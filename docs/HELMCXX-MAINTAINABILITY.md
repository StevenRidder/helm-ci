...
| `helm-server` previously lacked an explicit `target_compile_features(... cxx_std_17)` line in `0003-cli-cmakelists-helm-targets.patch`. | Cleared by ENGINE-20; the maintainability audit now verifies the target has an explicit C++17 compile feature. |
