...
- **Change Date:** 2030-06-27 for the initial public-alpha release, then the
  **fourth anniversary** of the publication of each later released version
  (rolling per-version, MariaDB-style).
...
- A `LICENSE` + `LICENSE.BSL` header policy per repo, committed before any repo goes public; extend to a file-level header policy per repo before any broad external contribution drive.
