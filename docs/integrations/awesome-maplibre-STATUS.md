...
_Last updated: 2026-06-25 · Branch: `claude/awesome-maplibre-exploration-acctbi` · Commit: `273f31d`_

> **Productionization update (2026-06-25).** The handoff step 3 below ("provide demo
> data") is now **done with real-format local data** — every integration renders genuine
> content offline, no network, no GDAL/tippecanoe. Generated by `pipeline/gen_demo_data.py`
> (pure stdlib): a Terrarium DEM pyramid (plausible Key West bathymetry), depth contours
> (marching squares → base-chart `depcnt` S-57 `VALDCO` + Lab geojson), soundings (`soundg`
> `DEPTH`), depth-area fill (`depare` `DRVAL1`), a 448-vessel AIS fleet, a 6-frame precip
> radar pack, places + wind field, a **hand-rolled PMTiles v3** archive, and a **hand-authored
> float32 GeoTIFF** for `cog://`. `web/integrations/*.js` now point at the local packs (cloud
> source documented as the production swap); depth contours were promoted out of the Lab into
> the **base chart**. Data + schema verified headless (Playwright, 13/13, zero page errors).
> See `web/data/DEMO-DATA.md`. **Caveat:** this headless build can't paint/query runtime map
> layers (SwiftShader worker limit) — final pixel render is a real-browser check. **No GDAL
> is required** for any of this; it's optional pipeline tooling for real GRIB/raster ingest.
>
