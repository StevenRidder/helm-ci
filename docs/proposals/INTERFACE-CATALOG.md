...
| Sat-first region bundle profile | [interfaces/region-bundle-sat-first-v1.md](interfaces/region-bundle-sat-first-v1.md) | `helm-packd`, `pipeline/region_bundle.py` | offline drawer, bake tooling, clients | Constrains OFFLINE-8 manifests for North Star sat-first offline packs. |
| Chart intake & library | [interfaces/chart-intake-v1.md](interfaces/chart-intake-v1.md) | `helm chart import` CLI, `helm-packd`, `helm-server` | cockpit Chart Library panel, `/catalog`, offline tooling | Owns BYO chart intake: canonical library folder, type-routed import, ENC-vs-tile render boundary. |
