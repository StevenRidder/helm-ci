"""Repair queued BOYSPH Chart No.1 parity rows into owned batch 67.

Run:
  python3 -m forge.standard_repair_batch59 --render
"""
...
OUT = ROOT / "out" / "standard_repair_batch59"
SVG_OUT = ROOT / "assets" / "svg" / "owned_repair_batch67"
REPORT = CATALOG / "owned_repair_batch67.json"
SUMMARY = CATALOG / "owned_repair_batch67.md"
...
def _svg(asset: str, body: str) -> str:
    return (
        ...
        'data-repair-batch="standard-repair-batch59">'
        f"<title>{asset} spherical buoy parity repair candidate</title>"
...
def _sphere(asset: str, row: dict) -> str:
    colours = _colours(row)
    clip_id = f"clip_{_safe(asset)}"
    parts = [f'<defs><clipPath id="{clip_id}"><circle cx="32" cy="32" r="18"/></clipPath></defs>']
    if _pattern(row) == "vertical":
        width = 36 / len(colours)
        for index, colour in enumerate(colours):
            parts.append(
                f'<rect x="{14 + index * width:.2f}" y="14" width="{width:.2f}" height="36" '
                f'fill="{_colour(colour)}" clip-path="url(#{clip_id})" data-pattern="vertical-sphere-band"/>'
            )
    else:
...
def _cone(asset: str, row: dict) -> str:
    colours = _colours(row)
    ...
    for index, colour in enumerate(colours):
        parts.append(
            ...
            f'fill="{_colour(colour)}" clip-path="url(#{clip_id})" data-pattern="horizontal-cone-band"/>'
        )
...
def _body(asset: str, row: dict) -> str:
    if asset == "BOYSPH79":
        return _cone(asset, row)
    return _sphere(asset, row)
...
def _repair_items() -> list[dict]:
    ...
    items = [row for row in queue.get("items", []) if row.get("asset", "").startswith("BOYSPH")]
    if items:
        return items
    if REPORT.exists():
...
def build(*, render_outputs: bool = False) -> dict:
    ...
    items = _repair_items()
    if not items:
        raise RuntimeError("no BOYSPH rows in standard repair queue")
    ...
    for item in items:
        ...
        rows.append({
            ...
            "queue_action": "standard_chart1_parity_spherical_buoy_consumed",
            "risk_bucket": "chart1_spherical_buoy_repair_batch67",
            "candidate_strategy": "owned_spherical_buoy_redraw_from_s57_colour_order_and_chart1_parity_gate",
            ...
            "provenance": {
                ...
                "source_priority_basis": "standard_repair_queue Chart No.1 parity blocker for BOYSPH rows",
                "style_contract_id": OPENBRIDGE_STYLE_ID,
                "generator": "forge.standard_repair_batch59",
                "reference_role": "S-57 spherical/conical buoy metadata plus provider images drive generated-owned redraw",
            },
        })
    result = {
        "schema_version": 1,
...
def _write_summary(result: dict) -> None:
    lines = [
        "# Standard Repair Batch 59 / Owned Repair Batch 67",
        "",
        "Chart No.1 parity repair pass for queued `BOYSPH*` spherical buoy rows.",
        "",
    ...
    for row in result["symbols"]:
        ...
        shape = "cone" if row["asset"] == "BOYSPH79" else "sphere"
