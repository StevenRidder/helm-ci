"""Repair TOPSHP diagonal board slice into owned repair batch 49.

Run:
  python3 -m forge.standard_repair_batch41 --render
"""
...
OUT = ROOT / "out" / "standard_repair_batch41"
SVG_OUT = ROOT / "assets" / "svg" / "owned_repair_batch49"
REPORT = CATALOG / "owned_repair_batch49.json"
SUMMARY = CATALOG / "owned_repair_batch49.md"
PALETTES = ("day", "dusk", "night")
HOMEBREW_CAIRO = Path("/opt/homebrew/lib/libcairo.2.dylib")

REPAIRS = {
    "TOPSHP47": "board_red",
    "TOPSHP48": "board_green",
    "TOPSHP51": "diamond_white_black_white_vertical",
    "TOPSHP52": "diamond_black",
    "TOPSHP53": "diamond_yellow",
    "TOPSHP54": "diamond_red",
    "TOPSHP55": "diamond_orange",
    "TOPSHP58": "diamond_red_white_horizontal",
    "TOPSHP61": "diamond_orange_white_vertical",
    "TOPSHP62": "diamond_white_orange_vertical",
    "TOPSHP63": "diamond_white_red_white_vertical",
    "TOPSHP64": "diamond_white_green_white_checker",
    "TOPSHP65": "diamond_white_red_vertical",
    "TOPSHP67": "diamond_orange_white_vertical",
    "TOPSHP69": "diamond_white_red_vertical",
    "TOPSHP70": "diamond_yellow",
    "TOPSHP71": "diamond_white_orange_vertical",
    "TOPSHP72": "diamond_white_red_white_checker",
    "TOPSHP73;TE('%s'": "diamond_white_black_white_checker_text",
    "TOPSHP74": "diamond_red_white_white_red_checker",
}
...
def _svg(asset: str, body: str) -> str:
    return (
        ...
        'data-repair-batch="standard-repair-batch41">'
        f"<title>{asset} repair batch 49 TOPSHP diagonal board candidate</title>"
...
def _redraw(asset: str) -> str:
    ...
    if kind == "diamond_white_black_white_vertical":
        return _svg(asset, _diamond_board(asset, ["white", "black", "white"], pattern="vertical"))
    if kind == "diamond_black":
        return _svg(asset, _diamond_board(asset, ["black"]))
    if kind == "diamond_yellow":
        return _svg(asset, _diamond_board(asset, ["yellow"]))
    if kind == "diamond_red":
        return _svg(asset, _diamond_board(asset, ["red"]))
    if kind == "diamond_orange":
        return _svg(asset, _diamond_board(asset, ["orange"]))
    if kind == "diamond_red_white_horizontal":
        return _svg(asset, _diamond_board(asset, ["red", "white"], pattern="horizontal"))
    if kind == "diamond_orange_white_vertical":
        return _svg(asset, _diamond_board(asset, ["orange", "white"], pattern="vertical"))
    if kind == "diamond_white_orange_vertical":
        return _svg(asset, _diamond_board(asset, ["white", "orange"], pattern="vertical"))
    if kind == "diamond_white_red_white_vertical":
        return _svg(asset, _diamond_board(asset, ["white", "red", "white"], pattern="vertical"))
    if kind == "diamond_white_green_white_checker":
        return _svg(asset, _diamond_board(asset, ["white", "green", "white", "green"], pattern="checker"))
    if kind == "diamond_white_red_vertical":
        return _svg(asset, _diamond_board(asset, ["white", "red"], pattern="vertical"))
    if kind == "diamond_white_red_white_checker":
        return _svg(asset, _diamond_board(asset, ["white", "red", "white", "red"], pattern="checker"))
    if kind == "diamond_white_black_white_checker_text":
        return _svg(asset, _diamond_board(asset, ["white", "black", "white", "black"], pattern="checker", label="T"))
    if kind == "diamond_red_white_white_red_checker":
        return _svg(asset, _diamond_board(asset, ["red", "white", "white", "red"], pattern="checker"))
...
def build(*, render_outputs: bool = False) -> dict:
    ...
    for asset in REPAIRS:
        ...
        rows.append({
            ...
            "risk_bucket": "topshape_repair_batch49",
            "candidate_strategy": "owned_topshape_diagonal_board_redraw_from_semantic_brief_and_provider_witnesses",
            ...
            "provenance": {
                ...
                "generator": "forge.standard_repair_batch41",
...
def _write_summary(result: dict) -> None:
    lines = [
        "# Standard Repair Batch 41 / Owned Repair Batch 49",
        "",
        "Owned redraws for TOPSHP diagonal board judge-failure slice.",
        "",
    ]
