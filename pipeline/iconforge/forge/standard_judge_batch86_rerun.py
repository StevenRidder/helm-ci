"""Judge rerun for owned repair batch 86 precise repair slice.

Run:
  python3 -m forge.standard_judge_batch86_rerun
"""
...
OUT_JSON = CATALOG / "standard_judge_batch_086_rerun.json"
OUT_MD = CATALOG / "standard_judge_batch_086_rerun.md"
BATCH_ID = "standard_judge_batch_086_rerun"
CREATED_AT = "2026-07-03T00:00:00Z"
SOURCE_BATCH = "catalog/owned_repair_batch86.json"

PASS_ASSETS = {
    "NMKINF38": "notice mark now has the single diagonal slash for end of prohibition/regulation.",
    "NMKINF53": "notice mark now has the three vertical bars for three vessels side-by-side.",
    "SCALEB10": "one-mile scale bar now uses segmented alternating vertical marks.",
    "SCALEB11": "10-mile latitude scale bar now uses segmented black vertical marks.",
    "TOPMAR90": "pricken point-down topmark now includes stem, downward point, and side/barb detail.",
    "TOPMAR93": "pricken point-up topmark now includes stem, upward point, and side/barb detail.",
    "WATTUR02": "overfalls/eddies/breakers mark now uses three compact wavelets.",
}
...
def build() -> dict:
    ...
    result = {
        ...
        "evidence_notes": [
            "Judged current repaired rows from owned_repair_batch86.",
            "Pass means judge_pass_pending_final_approval only; this artifact grants zero final approvals.",
            "Rows were repaired from exact visual-judge instructions and checked against available OpenCPN witnesses.",
        ],
        "project": "vulkan",
...
def _write_md(result: dict) -> None:
    lines = [
        "# Standard Judge Batch 086 Rerun",
        "",
        "- Project: `vulkan`",
        "- Task: `FORGE-15`",
        "- Agent: `codex/FORGE-15-judge-loop-current`",
        "- Source batch: `owned_repair_batch86`",
