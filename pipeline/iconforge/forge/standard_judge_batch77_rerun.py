"""Judge rerun for owned repair batch 77 mixed navigation/tooling slice.

Run:
  python3 -m forge.standard_judge_batch77_rerun
"""
...
OUT_JSON = CATALOG / "standard_judge_batch_077_rerun.json"
OUT_MD = CATALOG / "standard_judge_batch_077_rerun.md"
BATCH_ID = "standard_judge_batch_077_rerun"
CREATED_AT = "2026-07-03T00:00:00Z"
SOURCE_BATCH = "catalog/owned_repair_batch77.json"

PASS_ASSETS = {
    "EVENTS02": "mariner event mark preserves the orange square-with-slash witness cue.",
    "HECMTR01": "100m hectometre point preserves the compact magenta diamond cue.",
    "HECMTR02": "1km hectometre point preserves the compact magenta diamond cue.",
    "HGWTMK01": "high-water mark preserves the black HW board-on-post witness family.",
    "ISODGR51": "isolated-danger symbol preserves the magenta circle-with-X cue.",
    "NOTMRK03": "information/recommendation notice mark preserves the blue square notice cue.",
    "OSPONE02": "one-minute ownship-vector mark preserves the short black dash cue.",
    "OSPSIX02": "six-minute ownship-vector mark preserves the short black dash cue.",
    "PLNPOS02": "planned-position crossline preserves the short red line cue.",
    "POSITN02": "ownship position fix preserves the orange crosshair cue.",
    "PRICKE03": "port-hand withy preserves the black withy beacon silhouette and side-branch cue.",
    "PRICKE04": "starboard-hand withy preserves the mirrored black withy beacon silhouette and side-branch cue.",
}
...
def build() -> dict:
    ...
    result = {
        ...
        "evidence_notes": [
            "Judged current repaired rows from owned_repair_batch77.",
            "Pass means judge_pass_pending_final_approval only; this artifact grants zero final approvals.",
            "Withy orientation was checked separately because side cues are load-bearing.",
        ],
        "project": "vulkan",
...
def _write_md(result: dict) -> None:
    lines = [
        "# Standard Judge Batch 077 Rerun",
        "",
        "- Project: `vulkan`",
        "- Task: `FORGE-15`",
        "- Agent: `codex/FORGE-15-judge-loop-current`",
        "- Source batch: `owned_repair_batch77`",
