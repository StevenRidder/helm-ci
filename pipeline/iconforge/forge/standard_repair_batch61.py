"""Repair queued BOYSPR Chart No.1 parity rows into owned batch 69.

Run:
  python3 -m forge.standard_repair_batch61 --render
"""
...
OUT = ROOT / "out" / "standard_repair_batch61"
SVG_OUT = ROOT / "assets" / "svg" / "owned_repair_batch69"
REPORT = CATALOG / "owned_repair_batch69.json"
SUMMARY = CATALOG / "owned_repair_batch69.md"
...
def _svg(asset: str, body: str) -> str:
    return (
        ...
        'data-repair-batch="standard-repair-batch61">'
        f"<title>{asset} spar buoy parity repair candidate</title>"
...
def _body(asset: str, row: dict) -> str:
    ...
    points = "28,8 36,8 40,54 24,54"
    clip_id = f"clip_{_safe(asset)}"
    parts = [
        f'<defs><clipPath id="{clip_id}"><polygon points="{points}"/></clipPath></defs>',
        f'<polygon points="{points}" fill="{_colour("white")}" stroke="none"/>',
    ]
    height = 46 / len(colours)
    for index, colour in enumerate(colours):
        parts.append(
            f'<rect x="23" y="{8 + index * height:.2f}" width="18" height="{height:.2f}" '
            f'fill="{_colour(colour)}" clip-path="url(#{clip_id})" data-pattern="horizontal-spar-band"/>'
        )
    parts.append(f'<polygon points="{points}" fill="none" stroke="{_colour("black")}" stroke-width="3"/>')
    parts.append(f'<path d="M32 54 V59" fill="none" stroke="{_colour("black")}" stroke-width="3"/>')
    return "".join(parts)
...
def _repair_items() -> list[dict]:
    ...
    items = [row for row in queue.get("items", []) if row.get("asset", "").startswith("BOYSPR")]
    if items:
        return items
    if REPORT.exists():
...
def build(*, render_outputs: bool = False) -> dict:
    ...
    items = _repair_items()
    if not items:
        raise RuntimeError("no BOYSPR rows in standard repair queue")
    ...
    for item in items:
        ...
        rows.append({
            ...
            "queue_action": "standard_chart1_parity_spar_buoy_consumed",
            "risk_bucket": "chart1_spar_buoy_repair_batch69",
            "candidate_strategy": "owned_spar_buoy_redraw_from_s57_colour_and_chart1_parity_gate",
            ...
            "provenance": {
                ...
                "source_priority_basis": "standard_repair_queue Chart No.1 parity blocker for BOYSPR rows",
                "style_contract_id": OPENBRIDGE_STYLE_ID,
                "generator": "forge.standard_repair_batch61",
                "reference_role": "S-57 spar buoy metadata plus provider images drive generated-owned redraw",
            },
        })
    result = {
        "schema_version": 1,
...
def _write_summary(result: dict) -> None:
    lines = [
        "# Standard Repair Batch 61 / Owned Repair Batch 69",
        "",
        "Chart No.1 parity repair pass for queued `BOYSPR*` spar buoy rows.",
        "",
