"""Repair VECWTR through-water vector failures into owned batch 85.

Run:
  python3 -m forge.standard_repair_batch85 --render
"""
...
OUT = ROOT / "out" / "standard_repair_batch85"
SVG_OUT = ROOT / "assets" / "svg" / "owned_repair_batch85"
REPORT = CATALOG / "owned_repair_batch85.json"
SUMMARY = CATALOG / "owned_repair_batch85.md"
PALETTES = ("day", "dusk", "night")
HOMEBREW_CAIRO = Path("/opt/homebrew/lib/libcairo.2.dylib")
SOURCE_JUDGE = "catalog/standard_judge_batch_080_rerun.json"

TARGETS = {
    "VECWTR01": "black_single_chevron_through_water",
    "VECWTR21": "green_single_chevron_through_water",
}
...
def _svg(asset: str, body: str) -> str:
    return (
        ...
        'data-repair-batch="standard-repair-batch85">'
        f"<title>{asset} through-water vector repair candidate</title>"
        '<g stroke-linecap="round" stroke-linejoin="round">'
        f"{body}</g></svg>\n"
    )


def _single_chevron(colour: str) -> str:
    return (
        f'<path d="M23 39 L32 28 L41 39" fill="none" '
        f'stroke="{_colour(colour)}" stroke-width="2.5"/>'
    )


def _body(asset: str) -> str:
    if asset == "VECWTR01":
        return _single_chevron("black")
    if asset == "VECWTR21":
        return _single_chevron("green")
...
def build(*, render_outputs: bool = False) -> dict:
    ...
    items = _repair_items()
    if not items:
        raise RuntimeError("no VECWTR rows in standard repair queue")
    ...
    for item in items:
        ...
        rows.append({
            ...
            "queue_action": "standard_judge_batch_080_failure_consumed",
            "risk_bucket": "through_water_vector_arrow_repair_batch85",
            "candidate_strategy": "owned_single_chevron_redraw_from_opencpn_reference",
            ...
            "provenance": {
                ...
                "source_priority_basis": "standard_judge_batch_080_rerun through-water vector feedback",
                "style_contract_id": OPENBRIDGE_STYLE_ID,
                "generator": "forge.standard_repair_batch85",
                "reference_role": "OpenCPN through-water vector witnesses and S-52 metadata drive generated-owned redraw",
            },
        })
    result = {
        "schema_version": 1,
...
def _write_summary(result: dict) -> None:
    lines = [
        "# Standard Repair Batch 85 / Owned Repair Batch 85",
        "",
        "Targeted through-water vector-arrow redraws for failures from `standard_judge_batch_080_rerun`.",
        "",
