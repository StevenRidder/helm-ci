...
def main() -> None:
    ...
    assert counts["not_colour_bearing"] > 0
    ...
    assert bcngen64["gate_status"] == "warn"
    assert bcngen64["render_colour_authority"] == "generated_visual_recipe"
    ...
    assert bcnlat15["gate_status"] == "warn"
    ...
    assert aisves01["visual_colour_sequence"] == []
    assert aisves01["visual_stroke_sequence"] == ["green", "green"]
    assert aisves01["status"] == "not_colour_bearing"
