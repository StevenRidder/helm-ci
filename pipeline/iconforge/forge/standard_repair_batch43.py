"""Repair remaining mixed TOPSHP topmark slice into owned repair batch 51.

Run:
  python3 -m forge.standard_repair_batch43 --render
"""
...
OUT = ROOT / "out" / "standard_repair_batch43"
SVG_OUT = ROOT / "assets" / "svg" / "owned_repair_batch51"
REPORT = CATALOG / "owned_repair_batch51.json"
SUMMARY = CATALOG / "owned_repair_batch51.md"
PALETTES = ("day", "dusk", "night")
HOMEBREW_CAIRO = Path("/opt/homebrew/lib/libcairo.2.dylib")

REPAIRS = {
    "TOPSHPA4": "slanted_red_white_vertical",
    "TOPSHPA5": "slanted_white_black_white_vertical",
    "TOPSHPA6": "slanted_red",
    "TOPSHPA7": "slanted_green_red_green_vertical",
    "TOPSHPA8": "slanted_green_black_vertical",
    "TOPSHPA9": "slanted_white_red_horizontal",
    "TOPSHPB0": "slanted_red_white_horizontal",
    "TOPSHPD1": "round_orange",
    "TOPSHPD2": "round_green",
    "TOPSHPD3": "round_red",
    "TOPSHPD5": "round_red_white_black",
    "TOPSHPI1": "x_yellow",
    "TOPSHPI2": "x_black",
    "TOPSHPI3": "x_white_red_diagonal",
    "TOPSHPJ1": "staff_yellow",
    "TOPSHPJ3": "staff_white",
    "TOPSHPP2": "staff_yellow",
    "TOPSHPR1": "staff_black",
    "TOPSHPS1": "staff_white_red_white",
    "TOPSHPT1": "slanted_red",
}
...
def _svg(asset: str, body: str) -> str:
    return (
        ...
        'data-repair-batch="standard-repair-batch43">'
        f"<title>{asset} repair batch 51 mixed TOPSHP topmark candidate</title>"
...
def _redraw(asset: str) -> str:
    ...
    if kind == "slanted_red":
        return _svg(asset, _slanted_board(asset, ["red"]))
    if kind == "slanted_green_red_green_vertical":
        return _svg(asset, _slanted_board(asset, ["green", "red", "green"], pattern="vertical"))
    if kind == "slanted_green_black_vertical":
        return _svg(asset, _slanted_board(asset, ["green", "black"], pattern="vertical"))
    if kind == "slanted_white_red_horizontal":
        return _svg(asset, _slanted_board(asset, ["white", "red"], pattern="horizontal"))
    if kind == "slanted_red_white_horizontal":
        return _svg(asset, _slanted_board(asset, ["red", "white"], pattern="horizontal"))
    if kind == "round_orange":
        return _svg(asset, _round_board(asset, ["orange"]))
    if kind == "round_green":
        return _svg(asset, _round_board(asset, ["green"]))
    if kind == "round_red":
        return _svg(asset, _round_board(asset, ["red"]))
    if kind == "round_red_white_black":
        return _svg(asset, _round_board(asset, ["red", "white", "black"], pattern="vertical"))
    if kind == "x_yellow":
        return _svg(asset, _x_topmark(["yellow"]))
    if kind == "x_black":
        return _svg(asset, _x_topmark(["black"]))
    if kind == "x_white_red_diagonal":
        return _svg(asset, _x_topmark(["white", "red"], diagonal=True))
    if kind == "staff_yellow":
        return _svg(asset, _staff(["yellow"]))
    if kind == "staff_white":
        return _svg(asset, _staff(["white"]))
    if kind == "staff_black":
        return _svg(asset, _staff(["black"]))
    if kind == "staff_white_red_white":
        return _svg(asset, _staff(["white", "red", "white"]))
...
def build(*, render_outputs: bool = False) -> dict:
    ...
    for asset in REPAIRS:
        ...
        rows.append({
            ...
            "risk_bucket": "topshape_repair_batch51",
            "candidate_strategy": "owned_mixed_topshape_redraw_from_semantic_brief_and_provider_witnesses",
            ...
            "provenance": {
                ...
                "generator": "forge.standard_repair_batch43",
...
def _write_summary(result: dict) -> None:
    lines = [
        "# Standard Repair Batch 43 / Owned Repair Batch 51",
        "",
        "Owned redraws for mixed TOPSHP topmark judge-failure slice.",
        "",
    ]
