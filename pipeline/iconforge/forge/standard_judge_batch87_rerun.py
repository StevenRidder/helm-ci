"""Judge rerun for owned repair batch 87 TOPSHP repair slice.

Run:
  python3 -m forge.standard_judge_batch87_rerun
"""
...
OUT_JSON = CATALOG / "standard_judge_batch_087_rerun.json"
OUT_MD = CATALOG / "standard_judge_batch_087_rerun.md"
BATCH_ID = "standard_judge_batch_087_rerun"
CREATED_AT = "2026-07-03T01:45:00Z"
SOURCE_BATCH = "catalog/owned_repair_batch87.json"

PASS_ASSETS = {
    "TOPSHP47": "topmark now uses the compact red square board witness shape.",
    "TOPSHP48": "topmark now uses the compact green square board witness shape.",
    "TOPSHPI3": "topmark now uses the red X/cross witness silhouette.",
    "TOPSHPJ1": "topmark now uses the yellow cup/crown witness silhouette.",
    "TOPSHPJ3": "topmark now uses the white cup/crown witness silhouette.",
    "TOPSHPP2": "topmark now uses the yellow plus witness shape with visible yellow interior.",
    "TOPSHPR1": "topmark now uses the black trapezoid witness silhouette.",
    "TOPSHPS1": "topmark now uses the red-white-red target witness shape.",
}
...
def _expected(row: dict) -> str:
    ...
    shape = brief.get("required_shape") or "exact TOPSHP witness silhouette"
...
def build() -> dict:
    ...
    result = {
        ...
        "evidence_notes": [
            "Judged current repaired rows from owned_repair_batch87.",
            "Pass means judge_pass_pending_final_approval only; this artifact grants zero final approvals.",
            "Rows were repaired from exact visual-judge instructions and checked against available OpenCPN TOPSHP witnesses.",
        ],
        "project": "vulkan",
...
def _write_md(result: dict) -> None:
    lines = [
        "# Standard Judge Batch 087 Rerun",
        "",
        "- Project: `vulkan`",
        "- Task: `FORGE-15`",
        "- Agent: `codex/FORGE-15-judge-loop-current`",
        "- Source batch: `owned_repair_batch87`",
