"""Repair queued BOYPIL Chart No.1 parity rows into owned batch 65.

Run:
  python3 -m forge.standard_repair_batch57 --render
"""
...
OUT = ROOT / "out" / "standard_repair_batch57"
SVG_OUT = ROOT / "assets" / "svg" / "owned_repair_batch65"
REPORT = CATALOG / "owned_repair_batch65.json"
SUMMARY = CATALOG / "owned_repair_batch65.md"
...
def _svg(asset: str, body: str) -> str:
    return (
        ...
        'data-repair-batch="standard-repair-batch57">'
        f"<title>{asset} pillar buoy parity repair candidate</title>"
...
def _horizontal_bands(asset: str, colours: list[str]) -> list[str]:
    points = _points()
    ...
    for index, colour in enumerate(colours):
        parts.append(
            f'<rect x="14" y="{10 + index * height:.2f}" width="36" height="{height:.2f}" '
            f'fill="{_colour(colour)}" clip-path="url(#{clip_id})" data-pattern="horizontal-pillar-band"/>'
        )
    return parts


def _vertical_bands(asset: str, colours: list[str]) -> list[str]:
    points = _points()
    clip_id = f"clip_{_safe(asset)}"
    parts = [f'<defs><clipPath id="{clip_id}"><polygon points="{points}"/></clipPath></defs>']
    width = 36 / len(colours)
    for index, colour in enumerate(colours):
        parts.append(
            f'<rect x="{14 + index * width:.2f}" y="10" width="{width:.2f}" height="40" '
            f'fill="{_colour(colour)}" clip-path="url(#{clip_id})" data-pattern="vertical-pillar-band"/>'
        )
    return parts


def _pillar(asset: str, row: dict) -> str:
    colours = _colours(row)
    points = _points()
    parts = _vertical_bands(asset, colours) if _pattern(row) == "vertical" else _horizontal_bands(asset, colours)
    ...
    parts.append(f'<path d="M25 56 H39" fill="none" stroke="{_colour("black")}" stroke-width="3"/>')
    return "".join(parts)
...
def _repair_items() -> list[dict]:
    ...
    items = [row for row in queue.get("items", []) if row.get("asset", "").startswith("BOYPIL")]
    if items:
        return items
    if REPORT.exists():
...
def build(*, render_outputs: bool = False) -> dict:
    ...
    items = _repair_items()
    if not items:
        raise RuntimeError("no BOYPIL rows in standard repair queue")
    ...
    for item in items:
        ...
        svg = _svg(asset, _pillar(asset, source_row))
        ...
        rows.append({
            ...
            "queue_action": "standard_chart1_parity_pillar_buoy_consumed",
            "risk_bucket": "chart1_pillar_buoy_repair_batch65",
            "candidate_strategy": "owned_pillar_buoy_redraw_from_s57_colour_order_and_chart1_parity_gate",
            ...
            "provenance": {
                ...
                "source_priority_basis": "standard_repair_queue Chart No.1 parity blocker for BOYPIL rows",
                "style_contract_id": OPENBRIDGE_STYLE_ID,
                "generator": "forge.standard_repair_batch57",
                "reference_role": "S-57 pillar buoy metadata plus provider images drive generated-owned redraw",
            },
        })
    result = {
        "schema_version": 1,
...
def _write_summary(result: dict) -> None:
    lines = [
        "# Standard Repair Batch 57 / Owned Repair Batch 65",
        "",
        "Chart No.1 parity repair pass for queued `BOYPIL*` pillar buoy rows.",
        "",
        f"- failed_repaired: `{result['summary']['failed_repaired']}`",
        "- visual_parity: `repaired_pending_judge_rerun`",
        "",
        "| Asset | Colours | Pattern |",
        "| --- | --- | --- |",
    ]
